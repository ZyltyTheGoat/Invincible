package net.mcreator.invincible_craft.mixin;
...
public abstract class PlayerAnimationMixin<T extends LivingEntity> {
	...
	public void setupPivot(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (entityIn instanceof Player player_)
			...
			return;
		InvincibleCraftModPlayerAnimationAPI.PlayerAnimation animation = InvincibleCraftModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null)
			return;
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		if (ageInTicks <= 0)
			return;
		if (!master.equals("invincible_craft")) {
			if (!InvincibleCraftModPlayerAnimationAPI.animations.isEmpty())
				InvincibleCraftModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel<T> model = (PlayerModel<T>) (Object) this;
		Player player = null;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			InvincibleCraftModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		InvincibleCraftModPlayerAnimationAPI.PlayerAnimation animation = InvincibleCraftModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = InvincibleCraftModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				InvincibleCraftMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			InvincibleCraftModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, InvincibleCraftModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			InvincibleCraftModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
			ModelPart modelPart = getModelPart(model, boneName);
			if (modelPart == null)
				continue;
			// Apply rotation
			Vec3 rotation = InvincibleCraftModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress, player);
			if (rotation != null) {
			...
			Vec3 position = InvincibleCraftModPlayerAnimationAPI.PlayerBone.interpolate(bone.positions, animationProgress, player);
			if (position != null) {
			...
			// Apply scale
			Vec3 scale = InvincibleCraftModPlayerAnimationAPI.PlayerBone.interpolate(bone.scales, animationProgress, player);
			if (scale != null) {
